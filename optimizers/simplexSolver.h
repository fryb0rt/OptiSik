...
class SimplexSolver {
    ...
    Result executeFloat (const Operation operation) const {
        ...
        findPivot (equations, costFunction, nonArtificialCount);
        while (pivot.has_value ()) {
            ...
            // Move pivot variable into basis
            equations[*minimumEqIndex].movePivotToBasis (
            *pivot, costFunction.coefs[*pivot]);
